...
(function () {
  ...
  function emptyStats() {
    return {
      ...
      currentLossStreak: 0, monthToDateBiasCost: 0
    };
  }
  ...
  function render() {
    ...
    root.appendChild(
      h("nav", { className: "cb-tabs" },
        ...
        h("button", {
          ...
          onClick: function () { currentTab = "history"; render(); }
        }, "History"),
        h("button", {
    ...
    /* Trade History tab: fetch trades on first open, then render table */
    if (currentTab === "history") {
      if (historyTrades === null) {
        root.appendChild(h("div", { className: "cb-loading" }, "Loading history\u2026"));
        msg({ type: "GET_TRADES" }).then(function (t) {
          historyTrades = Array.isArray(t) ? t : [];
          render();
        }).catch(function () {
          historyTrades = [];
          render();
      ...
      renderHistory(root, historyTrades);
      return;
    }
  ...
  /* ── Trade History ──────────────────────────────────── */
  function renderHistory(root, trades) {
    var container = h("div", { className: "cb-history-wrap" });
    root.appendChild(container);

    if (!trades || trades.length === 0) {
      container.appendChild(
        h("div", { className: "cb-empty" },
          h("p", null, "No trade history yet."),
          h("p", { className: "cb-hint" }, "Import a CSV or confirm trades to see them here.")
        )
      );
      return;
    }
    ...
    section.appendChild(h("div", { className: "cb-section-title" }, "Trade History"));
    ...
    var chronOrder = sorted.slice().reverse();
    var cumPl = [];
    var run = 0;
    for (var c = 0; c < chronOrder.length; c++) {
      run += chronOrder[c].pl != null ? chronOrder[c].pl : 0;
      cumPl.push(run);
    }
    for (var j = 0; j < sorted.length; j++) {
      var t = sorted[j];
      var ts = new Date(t.timestamp).toLocaleString(undefined, { dateStyle: "short", timeStyle: "short" });
      var action = (t.action || "").toString();
      if (action.toLowerCase) action = action.charAt(0).toUpperCase() + action.slice(1).toLowerCase();
      var qty = t.quantity != null ? Number(t.quantity) : "";
      var entry = t.price != null ? "$" + Number(t.price).toFixed(2) : "—";
      var exitVal = "";
      if (t.quantity != null && t.pl != null && Number(t.quantity) !== 0) {
        exitVal = "$" + (Number(t.price) + Number(t.pl) / Number(t.quantity)).toFixed(2);
      } else {
        exitVal = "—";
      }
      var pl = t.pl != null ? "$" + Number(t.pl).toFixed(2) : "—";
      var plNum = t.pl != null ? Number(t.pl) : 0;
      var balance = cumPl[sorted.length - 1 - j];
      ...
      row.innerHTML =
        ...
        "<td>" + entry + "</td>" +
        "<td>" + exitVal + "</td>" +
        "<td class=\"cb-pl-cell " + (plNum >= 0 ? "cb-pl-pos" : "cb-pl-neg") + "\">" + pl + "</td>" +
        "<td class=\"cb-balance-cell\">$" + (balance != null ? balance.toFixed(2) : "0.00") + "</td>";
      tbody.appendChild(row);
    }

    table.appendChild(tbody);
    var scrollWrap = document.createElement("div");
    scrollWrap.className = "cb-history-scroll";
